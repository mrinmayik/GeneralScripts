   ...
   foreach modc ( `seq 1 $#moddo` )
      ...
      if ( $moddo[$modc] == 1 ) then
         ...
         if ( "$session_name" != "" ) then
            set niiname_sessionlabel = _${session_name}
         else
            set niiname_sessionlabel = ""
         endif
         if ( "$part_name" != "" ) then
            set niiname_partlabel = _${part_name}
         else
            set niiname_partlabel = ""
         endif

         set modpaths = ` find $study_root_in/$subj/$session_name -type d -name "*$modnames[$modc]${niiname_partlabel}*" `
         ...
         foreach runnum ( `seq -w ${d}1 $modnums[$modc] ` ) #$modpaths
            ...
            if ( $modnames[$modc] == $funcName ) then
               set nii_name = ${subj}${niiname_sessionlabel}_task-${funcName}_run-${runnum}${niiname_partlabel}_bold
            else if ( $modnames[$modc] == fmap ) then
               set nii_name = ${subj}${niiname_sessionlabel}_task-${niidirname}_run-${runnum}${niiname_partlabel}_bold
            else
               set nii_name = ${subj}${niiname_sessionlabel}_run-${runnum}${niiname_partlabel}_$modnames[$modc]
            endif
